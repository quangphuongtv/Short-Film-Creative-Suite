...
function isValidGeminiApiKey(key: string): boolean {
  ...
  return trimmed !== "" && trimmed !== "undefined" && trimmed !== "null" && trimmed.length > 0;
}
...
function formatGeminiError(err: any): string {
  ...
  try {
    if (typeof err === 'object') {
      ...
      try {
        ...
        // Safe fallback if circular structure occurs
      }
    } else {
      errStr = String(err);
    }
  ...
  if (
    errStrLower.includes("exceeded its monthly spending cap") || 
    errStrLower.includes("monthly spending cap") || 
    errStrLower.includes("spending cap") ||
    errStrLower.includes("spending limit") ||
    errStrLower.includes("budget exceeded") ||
  ...
  if (errStrLower.includes("resource_exhausted") || errStrLower.includes("429")) {
  ...
  if (errStrLower.includes("api key not valid") || errStrLower.includes("api_key_invalid") || errStrLower.includes("api key invalid")) {
    return "Xác thực API Key thất bại: API Key của bạn không hợp lệ hoặc đã bị vô hiệu hóa. Vui lòng kiểm tra lại cấu hình khóa trong Google AI Studio.";
  }

  if (errStrLower.includes("quota") || errStrLower.includes("quota exceeded")) {
...
app.post("/api/verify-api-key", async (req, res) => {
  ...
  if (!isValidGeminiApiKey(apiKey)) {
    return res.status(400).json({
      valid: false,
      error: "Xác thực API Key thất bại: Bạn chưa nhập API Key hợp lệ."
    });
  }
