...
export default function ScriptInput({ onScriptParsed, onLoadSavedProject, globalBrief }: ScriptInputProps) {
  const [projectTitle, setProjectTitle] = useState(() => {
    if (globalBrief && (globalBrief.title || (globalBrief as any).Title)) {
      return globalBrief.title || (globalBrief as any).Title;
    }
  ...
  const [scriptText, setScriptText] = useState(() => {
    if (globalBrief && (globalBrief.scriptText || (globalBrief as any).ScriptText)) {
      return globalBrief.scriptText || (globalBrief as any).ScriptText;
    }
    return '';
  });
  const [aspectRatio, setAspectRatio] = useState(() => {
    if (globalBrief && (globalBrief.aspectRatio || (globalBrief as any).AspectRatio)) {
      return globalBrief.aspectRatio || (globalBrief as any).AspectRatio;
    }
    return '16:9';
  });
  const [visualStyle, setVisualStyle] = useState(() => {
    if (globalBrief && (globalBrief.visualStyle || (globalBrief as any).VisualStyle)) {
      return globalBrief.visualStyle || (globalBrief as any).VisualStyle;
    }
    return '3D Pixar';
  });
  const [language, setLanguage] = useState(() => {
    if (globalBrief && (globalBrief.language || (globalBrief as any).Language)) {
      return globalBrief.language || (globalBrief as any).Language;
    }
    return 'English';
  });
  ...
  React.useEffect(() => {
    if (globalBrief) {
      const titleVal = globalBrief.title || (globalBrief as any).Title || '';
      const scriptVal = globalBrief.scriptText || (globalBrief as any).ScriptText || '';
      const aspectVal = globalBrief.aspectRatio || (globalBrief as any).AspectRatio || '16:9';
      const visualVal = globalBrief.visualStyle || (globalBrief as any).VisualStyle || '3D Pixar';
      const langVal = globalBrief.language || (globalBrief as any).Language || 'English';

      setProjectTitle(titleVal);
      setScriptText(scriptVal);
      setAspectRatio(aspectVal);
      setVisualStyle(visualVal);
      setLanguage(langVal);
    }
  }, [globalBrief]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        // 1. Try parsing directly as standard JSON first
        try {
          data = JSON.parse(trimmedContent);
        } catch (jsonErr) {
          // 2. If it is wrapped in .ts/JS export block, parse the object from inside braces
          const startIndex = content.indexOf('{');
          const endIndex = content.lastIndexOf('}');
          if (startIndex === -1 || endIndex === -1) {
            throw new Error("Không tìm thấy cấu trúc đối tượng dữ liệu hợp lệ. File phải đúng định dạng JSON hoặc file Export lưu từ Suite.");
          }
          const rawObjStr = content.substring(startIndex, endIndex + 1);
          data = new Function(`return (${rawObjStr})`)();
        }

        // Unwrap standard wrappers like ES exported SAVED_PROJECT_DATA or default exports if they exist
        if (data && typeof data === 'object') {
          if (data.SAVED_PROJECT_DATA && typeof data.SAVED_PROJECT_DATA === 'object') {
            data = data.SAVED_PROJECT_DATA;
          }
          ...
          if (data.globalBrief) {
            ...
            throw new Error("Dữ liệu không chứa thông tin cấu hình kịch bản hợp lệ (thiếu trường 'globalBrief').");
          }
        } else {
          throw new Error("Định dạng dữ liệu không phải là đối tượng hợp lệ.");
        }
      } catch (err: any) {
        console.error(err);
        setError(`Lỗi nạp file tiến độ kịch bản: ${err.message}`);
      }
    };
      ...
      <div className="border-b border-slate-800 pb-4 mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center">
          <label className="flex items-center gap-2 py-2 px-4 bg-emerald-950/40 hover:bg-emerald-900/50 text-emerald-300 hover:text-emerald-100 border border-emerald-800/80 hover:border-emerald-700 rounded-lg font-mono text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-[0_0_15px_rgba(16,185,129,0.15)]">
            ...
            <span>TẢI KỊCH BẢN (.ts, .json)</span>
            <input
              type="file"
              accept=".ts,.js,.json"
